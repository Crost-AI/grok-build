...
The Grok Build repository is itself a marketplace (named `grok-build`) carrying the bundled plugins under `plugins/` — currently the [Discord channel](25-channels.md#the-bundled-discord-plugin): `grok plugin marketplace add lakerfan901/grok-build`, then `grok plugin install discord@grok-build --trust`.
