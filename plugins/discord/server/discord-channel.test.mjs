...
async function main() {
  ...
  const experimental = init.result?.capabilities?.experimental ?? {}
  check(
    ['grok/channel', 'codex/channel', 'claude/channel'].every((k) => experimental[k] !== undefined),
    'initialize declares the channel capability for all three namespaces',
  )
  check(
    experimental['claude/channel/permission'] !== undefined,
    'initialize declares the Claude permission-relay capability',
  )
  check(
    ['grok/channel', 'codex/channel', 'claude/channel'].every((k) => {
      const commands = experimental[k]?.commands
      return (
        ...
        commands?.extra_args?.interaction_token === 'interaction_token'
      )
    }),
    'every namespace capability carries the slash-command reply descriptor',
  )
  check(
  ...
  // Multi-namespace fan-out: the same event is emitted once per served
  // namespace so any of the three hosts can consume it.
  for (const ns of ['codex', 'claude']) {
    const twin = await expectStdout(
      (m) => m.method === `notifications/${ns}/channel` && m.params.meta.message_id === 'm1',
      `${ns} twin of the DM event`,
    )
    check(
      twin.params.content === ev.params.content &&
        JSON.stringify(twin.params.meta) === JSON.stringify(ev.params.meta),
      `${ns} namespace event is identical to the grok one`,
    )
  }
  ...
  await new Promise((r) => setTimeout(r, 300))
  const permPost = rest.requests.findLast(
    (r) =>
      r.method === 'POST' &&
      r.url === '/api/v10/channels/c1/messages' &&
      r.body?.content?.includes('req9'),
  )
  check(
    Boolean(permPost) && permPost.body.content.includes('Bash'),
    'permission request posted to the configured Discord channel',
  )
  gw.send({
    op: 0,
    s: 70,
    ...
    d: {
      ...
      guild_id: 'g1',
      channel_id: 'c1',
      content: '<@BOT> yes req9',
      author: { id: '42', username: 'karl' },
      mentions: [{ id: 'BOT' }],
    },
  })
  const permNotif = await expectStdout(
    (m) => m.method === 'notifications/claude/channel/permission',
    'permission decision notification',
  )
  check(
    permNotif.params.request_id === 'req9' && permNotif.params.behavior === 'allow',
    'yes-reply returns behavior allow with the request id',
  )
  check(
    !fromBridge.some(
